...
const db = (): TransactionalSqlExecutor => {
  ...
  const query: SqlQueryExecutor['query'] = async (sql, params = []) => {
    ...
    if (sql.includes('INSERT INTO prodx_shift_operations')) {
      if (!opInserted) return { rows: [] as unknown[] };
      opInserted = false; opId = String(params[0]); return { rows: [{ id: opId }] as unknown[] };
    }
    if (sql.includes('SELECT id,payload_hash') && sql.includes('prodx_shift_operations')) return { rows: [{ id: opId, payload_hash: 'different' }] as unknown[] };
    ...
    if (sql.includes('SELECT s.id,s.store_id')) return { rows: [{ id:'shift-1',store_id:'store-1',register_id:'reg-1',cashier_id:'user-1',cashier_name:'Cashier',opened_at:new Date().toISOString(),closed_at:null,status:'open',opening_float_amount:'100.00',actual_counted_cash_amount:null,currency:'THB' }] as unknown[] };
    if (sql.includes('FROM prodx_cash_movements')) return { rows: [{ id:'mov-1',shift_id:'shift-1',type:'opening_float',amount:'100.00',reason:'Initial opening cash drawer float',performed_by_user_id:'user-1',currency:'THB',created_at:new Date().toISOString() }] as unknown[] };
    return { rows: [] };
  };
