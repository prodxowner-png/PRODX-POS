...
export const createOrderService=(db:TransactionalSqlExecutor)=>({
  ...
  async getOrder(storeId:string,orderId:string){
    ...
    if(!row) return null;
    return (await (await import('./checkout-service')).orderFromDb(db,row,false)).order;
  },
  async voidOrder(input:{storeId:string;orderId:string;reason:string;requesterUserId:string;requesterSessionId:string;authorizationToken:string}){
    ...
    return db.transaction(async tx=>{
      ...
      for(const item of items){
        ...
        await tx.query(`INSERT INTO prodx_inventory_ledger(id,organization_id,store_id,product_id,quantity_delta,resulting_stock,reason,reference_id,performed_by_user_id) VALUES($1,$2,$3,$4,$5,$6,'audit_count_adjustment',$7,$8)`,[crypto.randomUUID(),row.organization_id,input.storeId,item.product_id,item.quantity,stock.current_stock,input.orderId,auth.supervisorUserId]);
      }
      ...
      return (await (await import('./checkout-service')).orderFromDb(tx,updated,false)).order;
    });
  }
});