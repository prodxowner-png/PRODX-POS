...
import { createSupervisorAuthorizationService } from '../auth/supervisor-authorization';

export class OrderVoidError extends Error {
  constructor(
    public readonly code: 'ORDER_NOT_FOUND' | 'ORDER_ALREADY_FINALIZED' | 'VOID_INVALID' = 'VOID_INVALID',
    message = 'Unable to void order.',
  ) { super(message); }
}

const authorizationDb = (tx: SqlQueryExecutor) => ({
...
export const createOrderService = (db: TransactionalSqlExecutor) => ({
  async getOrders(storeId: string, limit = 100) {
    ...
    return Promise.all(rows.map(async (row) => (await orderFromDb(db, row, false)).order));
  },
  ...
  async voidOrder(input: {
    storeId: string; orderId: string; reason: string; requesterUserId: string;
    requesterSessionId: string; authorizationToken: string;
  }) {
    if (!input.reason.trim() || !input.authorizationToken.trim()) {
      throw new OrderVoidError('VOID_INVALID', 'Void reason and supervisor authorization are required.');
    }
    return db.transaction(async (tx) => {
      ...
      const auth = await createSupervisorAuthorizationService(authorizationDb(tx)).consume({
        token: input.authorizationToken, organizationId: row.organization_id, storeId: input.storeId,
        requesterUserId: input.requesterUserId, requesterSessionId: input.requesterSessionId,
        action: 'void', orderId: input.orderId,
      });
