...
type ProductRow = {
  ...
  category_id: string; price_amount: string; cost_price_amount: string;
  currency: string; tax_rate_bps: number; current_stock: number;
  reorder_point: number; unit_of_measure: string; active: boolean;
};
...
const moneyCents = (value: string): number => {
  ...
  return Math.sign(whole) * (whole * 100 + fraction);
};
...
export const registerCatalogRoute = (
  ...
  app.get('/api/v1/catalog/products', requirePermission(permission), async (request: Request, response: Response) => {
    ...
    const rows = (await db.query<ProductRow>(
      `SELECT id,store_id,sku,barcode,name,category_id,price_amount::text,cost_price_amount::text,currency,tax_rate_bps,current_stock,reorder_point,unit_of_measure,active
       FROM prodx_products WHERE ${conditions.join(' AND ')} ORDER BY name, id`,
      params,
    )).rows;
  ...
  app.get('/api/v1/catalog/products/by-barcode/:barcode', requirePermission(permission), async (request: Request, response: Response) => {
    ...
    if (!barcode) {
      response.status(400).json({ error: { code: 'BARCODE_REQUIRED', message: 'Barcode is required.', requestId: request.id } });
      return;
    }
    const row = (await db.query<ProductRow>(
      `SELECT id,store_id,sku,barcode,name,category_id,price_amount::text,cost_price_amount::text,currency,tax_rate_bps,current_stock,reorder_point,unit_of_measure,active
       FROM prodx_products WHERE store_id=$1 AND barcode=$2 AND active=true LIMIT 1`,
      [context.principal.storeId, barcode],
    )).rows[0];
